...
TEST_F(selection_sort_fixture, stl_sort_vec) {
  algol::algorithms::sort::stl_selection_sort(std::begin(vec), std::end(vec));
  ASSERT_EQ(vec[0],  -3);
  ASSERT_EQ(vec,  sorted_vec);
}

TEST_F(selection_sort_fixture, stl_sort_string) {
  algol::algorithms::sort::stl_selection_sort(std::begin(str), std::end(str));
  ASSERT_EQ(str[0], 'A');
  ASSERT_EQ(str,  sorted_str);
}

TEST_F(selection_sort_fixture, stl_sort_array) {
  algol::algorithms::sort::stl_selection_sort(std::begin(array), std::end(array));
  ASSERT_EQ(array[0],  -3);
  ASSERT_EQ(array,  sorted_array);
}

TEST_F(selection_sort_fixture, stl_sort_list) {
  algol::algorithms::sort::stl_selection_sort(std::begin(lst), std::end(lst));
  ASSERT_EQ(lst,  sorted_lst);
}

TEST_F(selection_sort_fixture, stl_sort_char) {
  algol::algorithms::sort::stl_selection_sort(std::begin(x), std::end(x));
  ASSERT_EQ(x,  xs);
}
