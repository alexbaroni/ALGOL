...
namespace algol { namespace integer {
    ...
    {
        integer_interval<T>& ival_;
        mutable T obj_;

        bool next() const
        {
            if (empty(ival_) || obj_ >= upper(ival_))
              return false;
    ...
    public:
        ...
        explicit integer_interval_range(integer_interval<T>& ival)
          : ival_(ival), obj_(lower(ival))
        {
        }
        ...
        {
            return !empty(ival_) && obj_ < upper(ival_);
        }

        bool operator!() const { return empty(ival_) || obj_ >= upper(ival_); }
    };
}}
