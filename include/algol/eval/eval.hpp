...
namespace algol {
  namespace eval {
    ...
    algol::result<T> evaluate_postfix (std::string const& expression)
    {
      algol::ds::fixed_stack<T, N> stack;
      eval_tokenizer postfix {expression};

      for (auto const& token : postfix) {
    ...
    algol::result<std::string> postfix_to_prefix (std::string const& expression)
    {
      algol::ds::fixed_stack<std::string, N> stack;
      std::istringstream postfix {expression};
    ...
    template <std::size_t N = 100>
    std::string prefix_to_postfix (std::string const& expression)
    {
      algol::ds::fixed_stack<std::string, N> stack;
