...
TEST_F(operation_counter_fixture, test_eq_op_count) {
  ...
  c = c;
  EXPECT_EQ(c.value(), 2);
  EXPECT_EQ(operation_counter::assignments(), static_cast<operation_counter::counter_type>(3));
}
...
TEST_F(operation_counter_fixture, emplace)
{
  ...
  EXPECT_EQ(operation_counter_vec::assignments(), static_cast<operation_counter_vec::counter_type>(2));
}
